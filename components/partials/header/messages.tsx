                    ...
                    <ScrollArea className="h-full">
                        {messages?.map((item: Message, index: number) => (
                            <DropdownMenuItem key={`inbox-${index}`} className="flex  items-start gap-3 py-2 px-4 cursor-pointer group ">
                                ...
                                {item.hasnotifaction && (
                                    <div className="flex-none">
                                        <span className="h-2.5 w-2.5 bg-destructive border border-destructive-foreground dark:border-default-400 rounded-full inline-block" />
                                    </div>
                                )}
                            </DropdownMenuItem>
                        ))}
