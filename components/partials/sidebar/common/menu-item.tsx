  ...
  if (config.sidebar === "draggable" && isDesktop) {
    return (
      <Button
        ...
        size={collapsed ? "icon" : "default"}
        className={cn(
          "group justify-start text-sm font-medium capitalize",
          "h-auto py-3 md:px-3 px-3",
        ...
        >
          {!collapsed && (
            <GripVertical
              {...attributes}
              {...listeners}
              className="inset-t-0 absolute me-1 h-5 w-5 ltr:-translate-x-6 rtl:translate-x-6 invisible opacity-0 transition-all group-hover:visible group-hover:opacity-100 group-hover:ltr:-translate-x-5 group-hover:rtl:translate-x-5"
            />
          )}
  ...
  if (config.sidebar === "compact" && isDesktop) {
    return (
      <Button
        ...
        color="secondary"
        className={cn(
          "flex-col h-auto py-1.5 px-3.5 capitalize font-semibold",
          "hover:ring-transparent hover:ring-offset-0",
          active
            ? "!bg-[#dcfce7] !text-[#00a63c] hover:!bg-[#dcfce7]"
            : "!bg-transparent hover:!bg-transparent",
