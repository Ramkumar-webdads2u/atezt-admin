...
import { getMenuList } from "@/lib/newMenus";
...
export function MenuClassic({ }) {
  ...
  const finalPath = pathname + (mode ? `?mode=${mode}` : "");
  const params = useParams<{ locale: string }>();
  const direction = getLangDir(params?.locale ?? "");
  const isDesktop = useMediaQuery("(min-width: 1280px)");

  // const menuList = getMenuList(finalPath, t, admin?.permission ?? []);
  const menuList = getMenuList(finalPath, t);
    ...
    <>
      ...
      <ScrollArea className="[&>div>div[style]]:block!" dir={direction}>
        ...
        <nav className="mt-5 h-full w-full">
          <ul className=" h-full flex flex-col min-h-[calc(100vh-48px-36px-16px-32px)] lg:min-h-[calc(100vh-32px-40px-32px)] items-start space-y-1 md:px-4">
